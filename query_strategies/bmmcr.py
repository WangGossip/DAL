...
# *BMMC+随机选择，根据当前选择的全部样本进行约束
# *b个结果投票进行选择，对于票数低于vote-line的再随机筛选，同时拿出一部分预算用于随机筛选
class BMMC_r(Strategy):
    def __init__(self, X, Y, idxs_lb, net, handler, args, device):
        super(BMMC_r, self).__init__(X, Y, idxs_lb, net, handler, args, device)
